    ...
    constructor(private tutorialService: TutorialService, 
        ...
        public deleteTutorial(): void {
             if (!this.tutorial.id) { 
                ...
                this.tutorialService.deleteTutorial(this.tutorial.id) .subscribe(response => { if (response) { this.router.navigate(['/tutorial/list']); } else { console.log(response); } }); } // Update tutorial status public updatePublished(status: boolean): void { if (!this.tutorial.id) { return; } this.tutorialService.updateTutorial(this.tutorial.id, this.tutorial) .subscribe(response => { if (response) { this.tutorial.published = status; } else { console.log(response); } }); } }
